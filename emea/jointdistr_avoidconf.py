...
from emea.indicator_avoid_conflict import ConflictIndicator
...
class AvoidConfJointDistri(nn.Module, JointDistrModule):
    def __init__(self, conf: Config):
        super(AvoidConfJointDistri, self).__init__()
        self.conf = conf
        ...
        self.train_alignment = test_alignment[:100]
        self.labelled_entities = train_alignment + self.train_alignment
        self.unlabelled_entities = np.array(test_alignment)[:, 0]

        self.one2one_checker = ConflictIndicator(conf)
    ...
    def set_stuff_about_neu_model(self, ent1_embs: np.ndarray, prob_mtx: torch.Tensor, pred_alignment:list, all_pred_alignment: list):
    ...
    def compute_features(self, sampled_alignment):
        ent_arr = np.arange(0, len(self.candidates))
        features = self.one2one_checker.compute_factors_in_batch(ent_arr, self.candidates, sampled_alignment, self.neu_all_pred_alignment)
        return features
